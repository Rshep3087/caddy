...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if s.shouldLogRequest(r) {
		...
		bodyReader := s.LengthReaderPool.Get().(*LengthReader)
		if r.Body != nil {
		...
		deferFunc := func() {
			s.logRequest(accLog, r, wrec, &duration, repl, bodyReader, shouldLogCredentials)
			s.LengthReaderPool.Put(bodyReader)
		}
